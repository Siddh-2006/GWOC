...
app.get('/', (req, res) => {
  res.json({
    service: 'MindSettler Keep-Alive Service (Simple)',
    status: 'running',
    uptime: Math.floor((Date.now() - stats.uptime) / 1000),
    version: '1.0.0',
    target: BACKEND_URL,
    pingInterval: PING_INTERVAL,
    approach: 'HTTP-based database monitoring'
  });
});
...
<body>
    <div class="container">
        <div class="header">
            <h1>🏥 MindSettler Keep-Alive Dashboard (Simple)</h1>
            <p>Monitoring: <strong>${BACKEND_URL}</strong></p>
            <p><em>HTTP-based database monitoring via backend health endpoint</em></p>
        </div>
